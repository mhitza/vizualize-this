...
sampleLoop source count t = do
  ...
  _ <- (P.simpleRead source $ sampleRate :: IO [GLfloat]) >>= renderS total >> return ()
  GLFW.swapBuffers
...
renderS count sampleS = do 
  _ <- get currentProgram >>= \p -> case p of
    Nothing -> return ()
    Just p  -> do tl <- get (uniformLocation p "time")
                  time <- getPOSIXTime >>= (return . fromIntegral . floor)
